...
function MysuccessWorkItem(props: MysuccessWorkItemProp) {
    ...
    return (
        <div className="relative w-full h-full overflow-hidden" onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)}>
            ...
            <Link target='_blank' href={data.linkWeb} className={`absolute w-full h-full left-0 ${isHover ? 'top-0' : 'top-60'} transition-all duration-300 bg-black rounded-lg bg-opacity-55 flex flex-col items-center justify-center text-white p-5 gap-3`}>
